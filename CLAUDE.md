...
- ✅ **Developer-loop phase** — closed. Bench at **5/5 (100%)** after two iteration passes ($2.67 total) with criterion pass rates at 100% across `loop_completed / iteration_budget_respected / files_in_scope_respected / no_regression` (the prior `cost_budget_respected` criterion was removed per [CONTRACTS.md C19](./docs/planning/2026-05-20-refinement/CONTRACTS.md) — all $-budgets dropped). p95 iterations = 1 (every fixture solved in a single Ralph iteration). Ralph loop runner ([`loops/ralph/runner.ts`](./loops/ralph/runner.ts)) accepts injectable `qualityGate` and `agent`; SDK adapter ([`loops/ralph/claude-agent.ts`](./loops/ralph/claude-agent.ts)) supports `disallowedTools` / `systemPrompt`. Shared invocation contract in [`orchestrator/dev-invocation.ts`](./orchestrator/dev-invocation.ts) (`buildDevSystemPrompt`, `prepareDevWorkspace`, `tallyToolUse`). `cycle.ts:runDeveloperLoop()` walks WIs in topological order, skips dependents of failed prerequisites, emits `ralph.start` / `ralph.end` per WI plus a phase-level summary. Bench under [`benchmarks/developer-loop/`](./benchmarks/developer-loop/) with five multi-language fixtures (Python/pytest, TS/node:test, bash/bats, doc/grep). Iteration cap aligned with phase-doc target: 3 iterations (healarr at 2). See [`brain/log.md`](./brain/log.md) for the full closure entry.
- ✅ **Review-loop phase (unified Ralph)** — closed end-to-end. Per-phase bench at **5/5 (100%)** ($2.55/run after refactor); e2e bench at **1/1 (100%)** ($1.18/run, score 1.0, merged on round 1). Stage 1 + stage 2 collapse into a single review-Ralph runner on the initiative branch — same `loops/ralph/runner.ts` infrastructure as the dev-loop, parameterised by reviewer system prompt + verdict-aware quality gate. Iteration 1 prepares the demo + PR draft; iterations 2+ react to send-back feedback in `fix_plan.md` (Ralph-style state, NOT WI-spec changes). Cap 3 iterations (1 prep + ≤2 send-back rounds); **no per-iteration $/turn budget guard** (removed 2026-05-18, ratified by C19 2026-05-23). Verdict-provider is injectable (`GetVerdict`) — production: **file-based** (`makeFileVerdict` in [`orchestrator/file-verdict.ts`](./orchestrator/file-verdict.ts); the operator writes `_queue/in-flight/<id>.verdict-response.md` via the `forge review <id>` CLI / `/forge-review`), bench: simulator. Ralph runner widened to support async `qualityGate`. **2026-05-23 unification (S4):** the dedicated reviewer skill collapsed — [`orchestrator/reviewer-stage2.ts`](./orchestrator/), `reviewer-invocation.ts`, `skills/reviewer/SKILL.md` and the per-phase `benchmarks/review-loop/` were archived; the loop runs as the developer-loop's **unifier** sub-phase ([`skills/developer-unifier/SKILL.md`](./skills/developer-unifier/SKILL.md)) plus the file-/PR-verdict providers, with the e2e bench as the regression gate.
