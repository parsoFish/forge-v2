...
export async function runCycleWithBoundedRetry(
  ...
  for (;;) {
    ...
    // Policy declined a retry (terminal kind / cap reached) OR we hit
    // the absolute attempt bound: terminal failure for the bench. The
    // prior bench-side override that rode `dev-loop-total-failure` as
    // recoverable was removed alongside the 14-mode taxonomy
    // (rebuild-review 2026-05-24 §3 #8); under transient|terminal the
    // production policy is authoritative.
    return { result, attempts, retriedModes };
  }
