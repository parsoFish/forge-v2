...
/**
 * ADR 020 — the focused architect hex for the dedicated plan screen. Reuses the
 * agent-flow hex aesthetic from AgentGraphCanvas (glowing hexagon + progress
 * arc), scoped to a single architect session. Amber glow = "needs you"
 * (awaiting answers / verdict); blue = the architect is working; pulses with
...
export function ArchitectStageHex({
  ...
  const glow = PHASE_GLOW[phase] ?? '#475059';
  const frac = PHASE_FRAC[phase] ?? 0;
  const recent = events.filter((e) => {
    const t = e.started_at;
    return t ? nowMs - new Date(t).getTime() < BURST_WINDOW_MS : false;
  });
  ...
  const active = working || recent.length > 0;

  return (
    <div
      data-component="architect-hex"
      data-architect-phase={phase}
    ...
    >
      ...
      <div style={{ fontSize: 13, fontWeight: 600, color: '#e6edf3', letterSpacing: 0.3 }}>architect</div>
      <div style={{ fontSize: 12, color: glow }}>{PHASE_LABEL[phase] ?? phase}</div>
      {/* Ephemeral tool bursts — thin chips that fade as they age out of the window. */}
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center', minHeight: 22 }}>
        {recent.slice(-6).map((e, i) => {
          const t = e.started_at;
          const age = t ? nowMs - new Date(t).getTime() : BURST_WINDOW_MS;
          ...
          return (
            ...
            >
              {(e.metadata?.tool_name as string) ?? e.event_type}
            </span>
          );
        })}
      </div>
      {lastTool && (
        <div style={{ fontSize: 11, color: '#6e7681', fontFamily: MONO }}>
          last: {(lastTool.metadata?.tool_name as string) ?? lastTool.message ?? 'tool'}
        </div>
      )}
    </div>
  );
...
function Hexagon({ size, glow, frac, active }: { size: number; glow: string; frac: number; active: boolean }): JSX.Element {
  ...
  const gid = `arch-arc-${glow.slice(1)}`;
  return (
    <svg
      width={size}
      height={size}
