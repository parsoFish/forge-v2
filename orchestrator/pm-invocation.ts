...
export function renderPmUserPrompt(input: PmUserPromptInput): string {
  return [
    ...
    '',
    `**You MUST \`Glob\` the actual project tree before drafting any WI.** Hallucinated \`files_in_scope\` paths are the single largest source of dev-loop failure: the architect describes a project in narrative form, the PM agent invents plausible-but-fictional paths from generic priors (\`src/engine/\`, \`src/core/\`, \`lib/\`, etc.), and the dev agent then wastes a budget either \`mkdir -p\`-ing the fabricated tree or hallucinating files inside it.`,
    '',
    'Required Globs (minimum) — run BEFORE drafting any WI:',
    `- \`Glob({ pattern: "${input.worktreeRelPath}/src/**" })\` — enumerate the entire source tree`,
    `- \`Glob({ pattern: "${input.worktreeRelPath}/tests/**" })\` (or \`spec/**\`, \`__tests__/**\` — try the conventional names) — enumerate existing tests`,
    `- \`Read({ file_path: "${input.worktreeRelPath}/package.json" })\` (or \`pyproject.toml\`, \`Cargo.toml\` — match the project) — confirm scripts, deps, project type`,
    `- \`Read({ file_path: "${input.worktreeRelPath}/README.md" })\` and \`CLAUDE.md\` if present — project conventions`,
    '',
    ...
    '## Inputs',
    '',
    `- Initiative manifest: \`${input.manifestRelPath}\` (read this AFTER the brain queries AND after the structural Globs).`,
    `- Worktree: \`${input.worktreeRelPath}/\` — already enumerated in Step 0.5. Read individual files (key src/* modules, README, package.json scripts) before deciding the file-by-file scope of each WI. The orchestrator validates every \`files_in_scope\` path post-emission and FAILS the cycle if you reference a path that doesn't exist and isn't being created — fabrication is detected.`,
    '',
    '## Output requirements',
    '',
