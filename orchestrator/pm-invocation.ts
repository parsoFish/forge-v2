...
export function renderPmUserPrompt(input: PmUserPromptInput): string {
  ...
  return [
    ...
    `- Produce **${input.minWorkItems}–${input.maxWorkItems}** work items. Forge earns its keep when an operator initiative is decomposed into a healthy DAG of small, parallel-friendly pieces — the whole reason for routing work through forge is that the busy-work parallelises. **A 1-WI initiative is a code-smell**: it usually means the architect over-narrowed the scope, or the work didn't need a cycle at all (the operator could have written it directly). If your draft has fewer than ${input.minWorkItems}, look for the seams you missed — a common decomposition is data-shape change → algorithm → integration → tests, or fixture → renderer → wiring → integration test. Stay under ${input.maxWorkItems} only because past that, the unifier starts to struggle to assemble; the failure mode of slight over-decomposition is much milder than the failure mode of under-decomposition.`,
    "- **Per-WI sizing (not a count cap).** If a single WI has more than 4 paths in `files_in_scope` OR more than 3 ACs OR mixes >2 distinct concerns (e.g. \"update fixture + wire renderer + update golden + integration test\"), it's too big — split it along the largest seam (fixture-only, render-only, test-only). This is *per-WI* guidance; it constrains the size of individual WIs but does NOT cap the total number of WIs. Splitting a too-large WI grows the count — that's the intended direction.",
    `- At least **${Math.round(input.parallelFractionAtLeast * 100)}%** of the work items must have an empty \`depends_on\` (i.e., be runnable in parallel from the start). Linear chains are an antipattern — find the independent slices. The dev-loop is built to run WIs in topological order, parallelising every level of the DAG; a chain of N WIs collapses that benefit to zero. If you genuinely cannot find parallel seams, question whether the initiative is small enough that the operator should just write it themselves.`,
