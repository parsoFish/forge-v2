...
3. For each feature in the initiative, decompose into work items:
   ...
   - **Each declares `creates:` listing every code file in `files_in_scope` it will modify or create** (or `verification_artifact:` for test-only WIs). This is MANDATORY for any WI whose `files_in_scope` contains a code file (anything except `.md` / `.txt` is treated as code, including `.tf`, `.hcl`, `.yaml`). The orchestrator's validator HARD-REJECTS WIs that violate this — the gate-tightening in `loops/ralph/stop-conditions.ts` uses `creates:` to verify the dev-loop agent actually touched the declared paths; without it, an agent can false-pass by leaving the file untouched. "Modify" counts — `creates:` does NOT mean "newly-created", it means "the file MUST appear in `git diff main...HEAD` after this WI's iteration completes".
...
5. **Practise file-scope discipline.** If two WIs would both edit the same file, prefer (a) splitting the file along the dimension that distinguishes them (one file per impl / concern), then (b) merging the WIs into one, then (c) adding a `depends_on` edge serialising them. **Each WI in the chain declares `creates: [<file>]`** — under the post-2026-05-24 validator rule (see step 3 above), every code-file WI must list its outputs. Two WIs sharing a file with no edge is a guaranteed merge conflict and the orchestrator's `detectHiddenCoupling()` validator will REJECT the cycle at PM close (the 2026-05-23 betterado dogfood failed exactly this way: WI-1 + WI-5 both listed a shared schema file with no edge → cycle failed at PM phase, $1.54 wasted).
