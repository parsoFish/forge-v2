...
- [`brain/forge/themes/quality-gate-cmd-must-assert-new-work.md`](../../brain/forge/themes/quality-gate-cmd-must-assert-new-work.md) — per-WI `quality_gate_cmd` MUST be sharp enough to fail on a clean tree (the agent's freedom to figure out HOW is bounded by a tight gate that proves the AC).
